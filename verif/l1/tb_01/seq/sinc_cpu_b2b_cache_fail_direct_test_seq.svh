...
// File        : sinc_cpu_b2b_cache_fail_direct_test_seq.svh
// Description : 

`ifndef SINC_CPU_B2B_CACHE_FAIL_DIRECT_TEST_SEQ
 `define SINC_CPU_B2B_CACHE_FAIL_DIRECT_TEST_SEQ

//##############################################################################
//<> SEQUENCE: sinc_cpu_b2b_cache_fail_direct_test_seq
//##############################################################################

/**
 * ECC Error Injection Test Sequence
 */
class sinc_cpu_b2b_cache_fail_direct_test_seq extends sinc_virtual_base_sequence;

  sinc_fault_err_packet m_fault_err_packet;
  
  `uvm_object_utils(sinc_cpu_b2b_cache_fail_direct_test_seq)
  ...
  function new(string name="sinc_cpu_b2b_cache_fail_direct_test_seq");
    super.new(name);
...
endclass : sinc_cpu_b2b_cache_fail_direct_test_seq

//##############################################################################
//<> Test Specific Functions
//##############################################################################

function void sinc_cpu_b2b_cache_fail_direct_test_seq::process_plusargs_and_populate_seq_item();
  string debug_str = "SINC_SEQ_PROCESS_PLUSARGS";
  string tmp_str;

endfunction : process_plusargs_and_populate_seq_item

task sinc_cpu_b2b_cache_fail_direct_test_seq::sinc_status_reg_err_check(input bit expect_hw_fault, input bit [7:0] expected_state);
...
task sinc_cpu_b2b_cache_fail_direct_test_seq::sequential_run_body();
  ...
  m_fault_err_packet.print_packet();
  ...
  m_cpu_rand_seq.m_cpu_mem_seq.ccpui_cpu_mem_read( .addr     (m_address[0]),
                                                   .loadstore(0),
                                                   .privmode (0),
                                                   .read_data(read_data));

  `uvm_info (get_name(), $sformatf("%s: test set 1 started, inject CACHE_FAIL when observe CPU_RD", debug_str), UVM_LOW)
  
  // start FSM error injection on the next state
  fork : inject_fault_error_cmu_ctrl
    begin
      m_top_configuration.m_sinc_vif.inject_fault_error_on_cmu_ctrl_fsm_state_illegal_on_ciu_state(m_force_on_ciu_cache_fsm_state, m_fault_err_packet.m_illegal_cmu_ctrl_fsm_state);
    end
  join_none

  // issue non blocking CPU reads
  m_cpu_rand_seq.m_cpu_mem_seq.ccpui_cpu_mem_nb_read(.addr(0), 
                                                     .loadstore(0), 
                                                     .privmode(0));
  
  m_cpu_rand_seq.m_cpu_mem_seq.ccpui_cpu_mem_nb_read(.addr(m_address[1]), 
                                                     .loadstore(0), 
                                                     .privmode(0));

  m_cpu_rand_seq.m_cpu_mem_seq.ccpui_cpu_mem_nb_read(.addr(m_address[2]), 
						     .loadstore(0), 
                                                     .privmode(0));

  m_cpu_rand_seq.m_cpu_mem_seq.ccpui_cpu_mem_nb_read(.addr(m_address[3]), 
						     .loadstore(0), 
                                                     .privmode(0));

  m_cpu_rand_seq.m_cpu_mem_seq.ccpui_cpu_mem_nb_read(.addr(m_address[4]), 
						     .loadstore(0), 
						     .privmode(0));

  m_cpu_rand_seq.m_cpu_mem_seq.ccpui_cpu_mem_nb_read(.addr(m_address[5]), 
						     .loadstore(0), 
						     .privmode(0));


  m_cpu_rand_seq.m_cpu_mem_seq.ccpui_cpu_mem_read( .addr     (m_address[2]),
                                                   .loadstore(0),
                                                   .privmode (0),
                                                   .read_data(read_data));
  ...
  `uvm_info (get_name(), $sformatf("%s: test set 2 started, inject CACHE_FAIL at same time of CPU_RD", debug_str), UVM_LOW)
  
   // start FSM error injection on the next state
  fork : inject_fault_error_cmu_ctrl_at_same_time
    begin
      m_top_configuration.m_sinc_vif.inject_fault_error_on_cmu_ctrl_fsm_state_illegal(m_fault_err_packet.m_illegal_cmu_ctrl_fsm_state);
    end
    begin
      wait (m_top_configuration.m_sinc_vif.is_fsm_fault_err_injected == 1);
       wait_n_clks(1);
      // issue non blocking CPU reads
      m_cpu_rand_seq.m_cpu_mem_seq.ccpui_cpu_mem_nb_read(.addr(m_address[0]), 
                                                     .loadstore(0), 
                                                     .privmode(0));
      ...
      m_cpu_rand_seq.m_cpu_mem_seq.ccpui_cpu_mem_read( .addr     (m_address[3]),
                                                   .loadstore(0),
                                                   .privmode (0),
                                                   .read_data(read_data));
...
//<> Extern Functions
//##############################################################################
task sinc_cpu_b2b_cache_fail_direct_test_seq::start_erase_optional();
...
task sinc_cpu_b2b_cache_fail_direct_test_seq::warm_reset();
...
`endif // SINC_CPU_B2B_CACHE_FAIL_DIRECT_TEST_SEQ
