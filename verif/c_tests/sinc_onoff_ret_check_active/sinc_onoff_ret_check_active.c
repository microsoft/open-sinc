...
// File          : sinc_onoff_ret_check_active.c
// Description   : C test that verifies retention state across power on/off in the active state
...
uint32_t set;
uint32_t tag_table[4];
uint32_t valid_table[4];
uint32_t tag_table_counter;
uint32_t block_table[10];
uint32_t exp_miss;
uint32_t exp_hit;

uint32_t block_in_tag_table(uint32_t tag)
{
...
uint32_t check_hit(uint32_t block_table_index)
{
  uint32_t tag, counter;

  tag = block_table[block_table_index];

  if(block_in_tag_table(tag))
  {
    return 1;
  }
  else
  {
    counter = tag_table_counter;
    tag_table[counter] = tag;
    valid_table[counter] = 1;
    if(counter==3)
    {
      tag_table_counter = 0;
    }
...
uint32_t access_rand_word(SINC_DRIVER * sinc_dvr, uint32_t block_table_index, uint32_t initial_value)
{
  uint32_t errors = 0;
  uint32_t tag = block_table[block_table_index];
  uint32_t word_num, offset_addr, rdata, exp_rdata;
  
  word_num = rand() % 128;
  if(check_hit(block_table_index))
  {
    exp_hit++;
  }
  else
...
VOID RUN_TEST(UINT32 iteration) {
  ...
  uint64_t dmb_addr;
  uint32_t errors = 0;
  uint32_t key_attr, tag, block_table_index, key_slot, dis_val;
  ...
  uint32_t initial_value, initial_value_mpu, offset_addr;
  ...
  //set key slot to non zero value
  key_slot = (rand() % (SSY_HSP_KSU_NUM_KEYS-1)) + 1;

  errors+=ccs_cmd.gen_random_key_fixed_attr(key_slot,key_attr);
  ...
  //pick 1 random sets and 10 random blocks within the set
  ...
  for(uint32_t i=0;i<10;i++)
  {
    ...
    do{
      tag = (rand() >> 16) & 0xff;
    } while(repeat_val(tag,i,block_table));
    block_table[i] = tag;
  }
  ...
  for(uint32_t i=0;i<10;i++)
  {
    offset_addr = ((block_table[i] & 0xff)<<16) | ((set & 0x7f)<<9);
  ...
  hw_status("Accessing 30 random words\n");
  for(uint32_t i=0;i<30;i++)
  {
    block_table_index = rand() % 9;
    errors+=access_rand_word(&sinc_dvr, block_table_index,initial_value);
  }
  ...
  hw_status("Modifying key in KSU\n");
  ...
  //check perf counters cleared
  sinc_dvr.write_perf_cntr_ctrl(SINC_PERF_HIT_CNTR_EN|SINC_PERF_MISS_CNTR_EN|SINC_PERF_LAT_CNTR_EN);
  exp_miss = 0;
  exp_hit = 0;
  errors+=sinc_dvr.check_perf_counters(exp_miss,exp_hit);

  //read last prior block
  errors+=access_rand_word(&sinc_dvr, block_table_index, initial_value);

  //read unread block
  errors+=access_rand_word(&sinc_dvr, 9, initial_value);
