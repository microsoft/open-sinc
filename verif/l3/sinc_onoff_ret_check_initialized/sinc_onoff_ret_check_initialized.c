...
// File          : sinc_onoff_ret_check_initialized.c
// Description   : C test that verifies retention state across power on/off in the initialized state
...
uint32_t access_rand_word(SINC_DRIVER * sinc_dvr, uint32_t set, uint32_t tag, uint32_t initial_value)
{
  uint32_t errors = 0;
...
VOID RUN_TEST(UINT32 iteration) {
  ...
  uint64_t dmb_addr;
  uint32_t errors = 0;
  uint32_t key_attr, tag, tag_index, key_slot, dis_val, set;
  ...
  uint32_t initial_value, initial_value_mpu, offset_addr, ciram_addr;
  ...
  for(uint32_t i=0;i<10;i++)
  {
    do{
      tag = rand() & 0xff;
    } while(repeat_val(tag,i,tag_table));
    tag_table[i] = tag;
  }
  ...
  //encrypt reproducible data into each block in last set
  initial_value = rand();
  for(uint32_t i=0;i<10;i++)
  {
    offset_addr = ((tag_table[i] & 0xff)<<16) | ((set & 0x7f)<<9);
  ...
  {
    tag_index = rand() % 10;
    errors+=access_rand_word(&sinc_dvr, set, tag_table[tag_index], initial_value);
  }
