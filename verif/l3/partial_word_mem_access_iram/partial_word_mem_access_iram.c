...
// File          : partial_word_mem_access_iram.c
// Description   : C test that exercises sub-word (byte/half-word) accesses to IRAM
...
VOID RUN_TEST(UINT32 iteration) {
  ...
  //note this isn't a sinc test, but needs to be present when sinc is if we run most tests out oc ciram
  //we may not be running a test that does partial word accesses to iram

  unsigned int errors = 0;
  uint8_t * data_8b = (uint8_t *) SSY_CPU0_MEMORY_DEF_HSP_IRAM_ADDRESS;
  uint32_t * data_32b = (uint32_t *) SSY_CPU0_MEMORY_DEF_HSP_IRAM_ADDRESS;
  uint32_t rdata;
