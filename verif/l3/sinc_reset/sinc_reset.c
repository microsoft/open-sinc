...
// File          : sinc_reset.c
// Description   : C test that verifies SInC behavior across reset
...
VOID RUN_TEST(UINT32 iteration) {
  ...
  if(hsp_reset_count == 0)
  {
    ...
    //check status for reset disabled
    sinc_dvr.read_status_check_bits(SINC_STATUS_SINC_RESET_DISABLED);

    //transition to initialized and try to do a reset
    errors+=sinc_dvr.transition_to_initialized();
    errors+=sinc_dvr.call_sinc_reset(SINC_STATUS_INVALID_CMD_ERR);
    errors+=sinc_dvr.check_sinc_state(SINC_STATE_INITIALIZED);

    //try to clear disable reset and then check that it's still disabled and we can't do an reset
  ...
  {
    //check status for reset disabled cleared by reset
    sinc_dvr.read_status_check_bits_not_set(SINC_STATUS_SINC_RESET_DISABLED);
  ...
  else if(hsp_reset_count == 2)
  {
    ...
    //transition to active and try to do a reset
    errors+=sinc_dvr.transition_to_initialized();
    errors+=sinc_dvr.transition_to_cache_active();
