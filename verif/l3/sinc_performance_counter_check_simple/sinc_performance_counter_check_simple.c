...
// File          : sinc_performance_counter_check_simple.c
// Description   : Simple variant of the performance counter check for quick smoke runs
...
uint32_t check_block(SINC_DRIVER * sinc_dvr, uint32_t block_num, uint32_t is_hit, uint32_t initial_value)
{
  uint32_t errors = 0;
  uint32_t offset_addr, rdata, exp_rdata;
  ...
  offset_addr = (block_num<<16);
  hw_status("Accessing address 0x%08x\n",offset_addr+SSY_CPU0_MEMORY_DEF_HSP_CIRAM_ADDRESS);
  rdata = hw_read32((uint32_t*)(SSY_CPU0_MEMORY_DEF_HSP_CIRAM_ADDRESS + offset_addr));
  exp_rdata = offset_addr + initial_value;
  if(rdata != exp_rdata)
  {
    hw_errmsg("compare saw mismatch read 0x%08x expected 0x%08x at addr 0x%08x\n",rdata,exp_rdata,(SSY_CPU0_MEMORY_DEF_HSP_CIRAM_ADDRESS + offset_addr));
    errors++;
  }

  if(sinc_dvr->read_miss_cntr_lower() != exp_miss)
  {
    hw_errmsg("exp_miss 0x%08x didn't match perf miss count 0x%08x\n",exp_miss,sinc_dvr->read_miss_cntr_lower());
    errors++;
  }
  else
  {
    hw_status("exp_miss is 0x%08x matched perf miss coun 0x%08x\n",exp_miss,sinc_dvr->read_miss_cntr_lower());
  }

  if(sinc_dvr->read_hit_cntr_lower() != exp_hit)
  {
    hw_errmsg("exp_hit 0x%08x didn't match perf hit count 0x%08x\n",exp_hit,sinc_dvr->read_hit_cntr_lower());
    errors++;
  }
  else
  {
    hw_status("exp_hit is 0x%08x matched perf hit count 0x%08x\n",exp_hit,sinc_dvr->read_hit_cntr_lower());
  }
  return errors;
}
...
VOID RUN_TEST(UINT32 iteration) {
  ...
  //encrypt reproducible data into 5 blocks in set 0
  initial_value = rand();
  ...
  //access blocks for first time, should all be misses
  for(uint32_t i=0;i<5;i++)
  {
    errors+=check_block(&sinc_dvr, i, 0, initial_value);
  }

  //retry most recent 4 blocks, should all be hits
  for(uint32_t i=1;i<5;i++)
  {
    errors+=check_block(&sinc_dvr, i, 1, initial_value);
  }

  //try block 0 again, should be another miss
  errors+=check_block(&sinc_dvr, 0, 0, initial_value);
